function findPath(graph, start, end) {
    const distance = {}; // distance[n] is n's dist from start (cumul)
    const parent = {};
    ...
    while (curr && curr != end) {
      visited.add(curr);
      for (const nbr in graph[curr]) { // add dists for graph nodes
        if (!visited.has(nbr)) {
      ...
      for (const node in distance) {
        if (!visited.has(node) && distance[node] < shortestDist) {
          // among unchecked nodes, which is closest to start?
    ...
    // backtrack through end's parents
    const path = [];
    let node = end;
    while (node) {
      path.unshift(node);
...
// an example i like
const path = findPath(g, '40748', '31240');
console.log(path);